function theta=Theta_ADMM(Y,W,H,X,L,alfa,beta,theta,maxiter)
...
GL=(X*L*X');%Graph Laplacian 
  ...
  while ~stop && iter < maxiter+1
      ...
      iter=iter+1;
      %solve theta
      theta=(mu*(H*X')+lamda1*X'+mu*G+lamda2+mu*(Q*X')+lamda3*X'+mu*(P*X')+lamda4*X')/(beta*GL+3*mu*(X*X')+mu*eye(size(X*X')));   
      %solve H
